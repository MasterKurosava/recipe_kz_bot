...
async def get_user_by_telegram_id(telegram_id: int, pool: asyncpg.Pool) -> Optional[Dict]:
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            "SELECT id, telegram_id, username, full_name, role FROM users WHERE telegram_id = $1",
            telegram_id
        )
        if row:
            return {
...
async def get_users_by_role(role: str, pool: asyncpg.Pool) -> List[Dict]:
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            "SELECT id, telegram_id, username, full_name, role FROM users WHERE role = $1 ORDER BY id",
            role
        )
        return [
            {
...
async def get_user_by_id(user_id: int, pool: asyncpg.Pool) -> Optional[Dict]:
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            "SELECT id, telegram_id, username, full_name, role FROM users WHERE id = $1",
            user_id
        )
        if row:
            return {
...
async def get_user_by_username(username: str, pool: asyncpg.Pool) -> Optional[Dict]:
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            "SELECT id, telegram_id, username, full_name, role FROM users WHERE username = $1",
            username
        )
        if row:
            return {
